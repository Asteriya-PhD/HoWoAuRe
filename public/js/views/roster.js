...
(function () {
  'use strict';
  const { api, toast, registerView, parseRosterSheet } = window.App;
  ...
  registerView('roster-view', {
    ...
    computed: {
      currentClass: {
        get() { return localStorage.getItem('hw.class') || (state.classes[0] && state.classes[0].id) || null; },
        set(v) { localStorage.setItem('hw.class', v || ''); },
      },
      classId() { return Number(this.currentClass) || (state.classes[0] && state.classes[0].id) || null; },
